...
func initialize(grid: UnitGrid, area: PlayArea):
	battle_grid = grid
	game_area = area
...
func get_next_position(moving_unit: BattleUnit, target_unit: BattleUnit) -> Vector2:
	...
	# STEP 1: remove the unit's current position as a solid and calc path
	...
	# STEP 2: when there isn't adj tile next to moving unit we stay put
	if path.size() == 1 and path[0] == unit_tile:
		astar_grid.set_point_solid(unit_tile, true)
		return Vector2(-1,-1)
		
	# STEP 3: valid next tile update grid, set next position to solid, and return coord
