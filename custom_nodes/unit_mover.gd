...
func _set_highlighters(enabled: bool):
	for play_area: PlayArea in play_areas:
		play_area.tile_highligher.enabled = enabled
...
func _reset_unit_to_starting_position(starting_position: Vector2, unit: Unit):
	var i := _get_play_area_from_position(starting_position)
...
func _on_unit_drag_started(unit: Unit):
	...
	var i := _get_play_area_from_position(unit.global_position)
	if i > -1:
...
func _on_unit_dropped(starting_position: Vector2, unit: Unit):
	_set_highlighters(false)
	var old_area_index := _get_play_area_from_position(starting_position)
	var drop_area_index := _get_play_area_from_position(unit.get_global_mouse_position())
