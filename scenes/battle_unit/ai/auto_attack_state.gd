...
func _init(new_actor: Node, current_target: BattleUnit):
	actor = new_actor
...
func _attack():
	...
	if actor_unit.stats.is_melee():
		...
		var projectile := actor_unit.range_attack.attack(target.global_position) as Projectile
...
func _on_detect_range_exited(area: Area2D):
